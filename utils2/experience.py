...
from utils2.binary_heap import BinaryHeap
...
class Experience(object):
    ...
    def __init__(self, conf):
        self.size = conf['memory']
        self.replace_flag = conf['per_replace_old'] if 'per_replace_old' in conf else True
        self.priority_size = conf['per_priority_size'] if 'per_priority_size' in conf else self.size

        self.alpha = conf['per_alpha'] if 'per_alpha' in conf else 0.7
        self.beta_zero = conf['per_beta_zero'] if 'per_beta_zero' in conf else 0.5
        self.batch_size = conf['per_batch_size'] if 'per_batch_size' in conf else 32
        self.learn_start = conf['per_learn_start'] if 'per_learn_start' in conf else 1000
        self.total_steps = conf['per_steps'] if 'per_steps' in conf else 100000
        # partition number N, split total size to N part
        self.partition_num = conf['per_partition_num'] if 'per_partition_num' in conf else 100
